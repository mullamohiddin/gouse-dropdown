...
function filterTree (filter, list)  {
  return _.filter(list, (item) => {
    if (item.items && item.items.length >0 ) {
     const items = filterTree(filter, item.items);
...
function (_Component) {
  ...
  _createClass(Dropdown, [{
    ...
    value: function filter(option) {
      ...
      var optionLabel =   filterTree(filterValue, option)
      return optionLabel;
    }
  }, {
    key: "hasFilter",
    ...
    value: function renderItems(selectedOption) {
      ...
      if (items && this.hasFilter()) {
        items = filterTree(this.state.filter, items);
      }
